...
pub fn build(b: *std.Build) void {
    ...
    const lib = buildWolfmqttLib(b, target, optimize, opts);
...
fn addModuleDefinitions(module: *std.Build.Module, opts: BuildOptions, target_info: std.Target) void {
    ...
    if (opts.broker) {
        ...
        // Broker features: define to enable (opt-in mode)
        if (opts.broker_retained) {
            module.addCMacro("WOLFMQTT_BROKER_RETAINED", "1");
        }
        if (opts.broker_will) {
            module.addCMacro("WOLFMQTT_BROKER_WILL", "1");
        }
        if (opts.broker_wildcards) {
            module.addCMacro("WOLFMQTT_BROKER_WILDCARDS", "1");
        }
        if (opts.broker_auth) {
            module.addCMacro("WOLFMQTT_BROKER_AUTH", "1");
        }
        if (opts.broker_log) {
            module.addCMacro("WOLFMQTT_BROKER_LOG", "1");
        }
        if (opts.broker_insecure) {
            module.addCMacro("WOLFMQTT_BROKER_INSECURE", "1");
        }
...
fn buildBroker(
    ...
    // 输出文件名添加 ABI 后缀
    const broker_name = b.fmt("mqtt_broker-{s}", .{@tagName(resolved_target.result.abi)});
    ...
    // 设置自定义安装路径: broker/<os>-<arch>
    const install_prefix = b.fmt("broker/{s}-{s}", .{
        @tagName(target.result.os.tag),
        @tagName(target.result.cpu.arch),
    });
    ...
    // 创建 broker 独立构建步骤
    const broker_step = b.step("broker", "Build and install broker executable");
    broker_step.dependOn(&install_step.step);
...
fn generateBuildReadme(b: *std.Build, name: []const u8, target: std.Target, opts: BuildOptions) void {
    const readme_content = b.fmt(
        ...
        \\| WOLFMQTT_BROKER | {s} |
        \\| WOLFMQTT_BROKER_RETAINED | {s} |
        \\| WOLFMQTT_BROKER_WILL | {s} |
        \\| WOLFMQTT_BROKER_WILDCARDS | {s} |
        \\| WOLFMQTT_BROKER_AUTH | {s} |
        \\| WOLFMQTT_BROKER_LOG | {s} |
        \\| WOLFMQTT_BROKER_INSECURE | {s} |
    ...
    // 写入 README.md 文件，文件名与构建产物一致（扩展名为 .md）
    const output_dir = b.fmt("{s}/{s}-{s}", .{
        name,
        @tagName(target.os.tag),
        @tagName(target.cpu.arch),
    });
    ...
    // 根据组件类型确定文件名
    const readme_filename = if (std.mem.eql(u8, name, "broker"))
        b.fmt("mqtt_broker-{s}.md", .{@tagName(target.abi)})
    else
        b.fmt("wolfmqtt-{s}-{s}.md", .{ @tagName(target.os.tag), @tagName(target.cpu.arch) });

    const readme_path = b.fmt("{s}/{s}", .{ output_dir, readme_filename });
