...
static const struct MqttPropMatrix gPropMatrix[] = {
    ...
    { MQTT_PROP_PAYLOAD_FORMAT_IND,         MQTT_DATA_TYPE_BYTE,
        (1 << MQTT_PACKET_TYPE_CONNECT) |    /* Will properties */
...
#ifdef WOLFMQTT_V5
            if (subscribe->protocol_level >= MQTT_CONNECT_PROTOCOL_LEVEL_5) {
                /* MQTT 5.0: Extract subscription options */
                /* Check if client explicitly set subscription options (bits 2-5 non-zero)
                 * or if all high bits are zero (indicating default values should be used) */
                if ((options & 0x3C) == 0) {
                    /* Bits 2-5 are all zero: client didn't specify options, use defaults */
                    topic->no_local = 0;           /* Default: No Local disabled */
                    topic->rap = 1;                 /* Default: preserve retain flag */
                    topic->retain_handling = 0;     /* Default: send retained messages */
                } else {
                    /* Client explicitly set subscription options */
                    topic->no_local = (options >> 2) & 0x01;
                    topic->rap = (options >> 3) & 0x01;
                    topic->retain_handling = (options >> 4) & 0x03;
                }
            } else {
                /* MQTT 3.1.1: Use default values for v5-only fields */
                topic->no_local = 0;
                topic->rap = 1;  /* Default: preserve retain flag */
                topic->retain_handling = 0;  /* Default: send retained messages */
            }
